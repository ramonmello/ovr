...
//-- instance = id da url adicionada na tabela mdl_page$content 
	$content ="<script>
		...
		function setStar(i,s){
			stars[s] = i;
			//CHAMAR AJAX COM O VALOR PARA ATUALIZAR
		}
		function colorStars(i,s){
			var j=1;
			for(;j<=5;++j){
				if(j<=i)
					document.getElementById(s+'STAR'+j).style.fill='yellow';
				else
	...
	for($i=0;$i<count($urls);++$i){		
		$content = $content.
					"<p style=\"\"text-align: center;\"\">"
						."<video id=\"\"".$i."\"\" controls=\"\"true\"\" src=\"\"".$urls[$i]."\"\">".$urls[$i]."</video>"
		...
		$content = $content."<svg width='20' height='20'>
			...
			onmouseover=\"\"colorStars(".$j.",".$i.");this.style.fill='yellow';\"\" onmouseout=\"\"colorStars(stars[".$i."],".$i.");\"\" onClick=\"\"setStar(".$j.",".$i.");updateVideoRating(".$i.",".$j.")\"\" />
		</svg>";
		}
		$content = $content."</p>";
	}
